...
def extract_subject(bookmaker_name: str, data: dict, league: str) -> Optional[tuple[str, str]]:
    ...
    if (participants_data := data.get('participants')) and (first_participants_data := participants_data[0]):
        ...
        if subject_name := first_participants_data.get('name'):
            ...
            subject_obj = utils.Subject(subject_name, league)
...
class OddsShopper(utils.BookmakerPlug):
    ...
    async def collect(self) -> None:
        ...
        url = utils.get_url(self.bookmaker_info.name, name='matchups')
                ...
                if offer_category.get('name') == 'PlayerProps':
                    ...
                    for league_data in offer_category.get('offers', []):
                        ...
                        if (league := extract_league(league_data)) and (offer_id := league_data.get('id')):
                            ...
                            url = utils.get_url(self.bookmaker_info.name).format(offer_id)
                            ...
                            params = utils.get_params(self.bookmaker_info.name, var_1=start_date, var_2=end_date)
