...
def extract_subject(data: dict, league: str) -> Optional[tuple[str, str]]:
    ...
    if subject := data.get('subject'):
        ...
        return get_subject_id(Subject(subject, league, position=extract_position(data),
                                      jersey_number=extract_jersey_number(data))), subject
...
def extract_market(data: dict, league: str) -> Optional[tuple[str, str]]:
    # make sure the data has enough elements to index properly
     if len(data) > 1:
        ...
        market = clean_market(data[1], league=league)
