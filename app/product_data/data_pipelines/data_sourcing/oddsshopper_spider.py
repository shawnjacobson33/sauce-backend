...
from app.product_data.data_pipelines.utils import RequestManager, DataStandardizer, clean_market, clean_subject, \
    clean_league, Helper, get_db, Market, Subject
...
class OddsShopperSpider:
    def __init__(self, batch_id: uuid.UUID, request_manager: RequestManager, data_standardizer: DataStandardizer):
        ...
        self.helper = Helper(bookmaker='OddsShopper')
        self.rm = request_manager
        self.ds = data_standardizer
        self.prop_lines = []

    async def start(self):
        url = self.helper.get_url(name='processing_info')
    ...
    async def _parse_processing_info(self, response):
        ...
        url = self.helper.get_url(name='matchups')
        ...
        for offer_category in data.get('offerCategories', []):
            ...
            if market_category == 'PlayerProps':
                for offer in offer_category.get('offers', []):
                    ...
                    url = self.helper.get_url().format(offer_id)
                    headers = self.helper.get_headers()
                    params = self.helper.get_params(var_1=start_date, var_2=end_date)
                    tasks.append(
...
async def main():
    ...
    spider = OddsShopperSpider(uuid.uuid4(), RequestManager(), DataStandardizer(batch_id, db))
