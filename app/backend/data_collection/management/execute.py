...
async def execute_roster_tasks(worker_names: list[str] = None) -> None:
    ...
    # get the coroutines
    roster_retrieving_tasks = ld.load_roster_tasks(worker_names)
...
async def execute_schedule_tasks(worker_names: list[str] = None) -> None:
    ...
    # get the coroutines
    schedule_tasks = ld.load_schedule_tasks(worker_names)
    # start making requests asynchronously
    t1 = time.time()
...
async def execute_box_score_tasks(worker_names: list[str] = None) -> None:
    ...
    # get the coroutines
    box_score_retrieving_tasks = ld.load_box_score_tasks(worker_names)
    # start making requests asynchronously
    t1 = time.time()
...
async def execute_line_tasks(group: str, worker_names: list[str] = None) -> None:
    ...
    # get the coroutines
    line_tasks = ld.load_line_tasks(group, worker_names)
    # start making requests asynchronously
    t1 = time.time()
