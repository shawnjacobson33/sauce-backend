...
async def _request_games(collected_games: list, league: str):
    ...
    for i in range(PAYLOAD['num_days_to_collect_from_league_map'][league]):
        ...
        if resp_html := await utils.requester.fetch(url, to_html=True, headers=headers, cookies=cookies):
            _parse_games(collected_games, resp_html, league, date)
...
def _extract_game_time_and_box_score_url(row, date: datetime) -> tuple[str, datetime] | None:
    if game_time_div := row.find('div', {'class': 'CellGame'}):
       if a_elem := game_time_div.find('a'):
           ...
           if 'scoreboard' not in box_score_url:
               game_time = _get_complete_date_time(date, a_elem.text)
...
def _parse_games(collected_games: list, html: str, league: str, requested_date: datetime):
    ...
    if divs := soup.find_all('div', {'class': 'TableBaseWrapper'}):
        if table := divs[-1].find('table', {'class': 'TableBase-table'}):
            if rows := table.find_all('tr'):
                for row in rows[1:]:
                    if game_info := _extract_game_time_and_box_score_url(row, requested_date):
                        box_score_url, game_time = game_info
                        if span_elems := row.find_all('span', {'class': 'TeamName'}):
                            if len(span_elems) > 1:
                                if away_team := _extract_team(span_elems[0]):
                                    if home_team := _extract_team(span_elems[1]):
...
async def run_basketball_games_collector(collected_games: list):
    tasks = []
    for league in CONFIGS['leagues_to_collect_from']:
        if utils.get_sport(league) == 'Basketball':
            tasks.append(_request_games(collected_games, league))

    await asyncio.gather(*tasks)


if __name__ == '__main__':
    asyncio.run(run_basketball_games_collector([]))
