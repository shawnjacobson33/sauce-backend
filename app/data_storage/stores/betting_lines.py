...
class BettingLines:
    ...
    def _get_game_id(self, league: str, team: str) -> Optional[str]:
        ...
        return self._r.hget(f'games:lookup:{league}', key=team_id)

    def get(self, league: str, team: str, key: str = None) -> Optional[Union[dict[str, str], str]]:
        if game_id := self._get_game_id(league, team):
            return self._r.hgetall(game_id) if not key else self._r.hget(game_id, key=key)
    ...
    def _get_live_ids(self) -> Optional[set[str]]:
        ...
        bl_ids = utils.get_live_ids(self._r, 'lines:watch:game_time')
        utils.update_live_hash(self._r, 'lines:index:live', bl_ids)
        return bl_ids

    def get_live(self) -> Optional[set[Union[str, dict[str, Any]]]]:
        if bl_ids := self._get_live_ids():
            return {self._r.hgetall(bl_id) for bl_id in bl_ids}  # TODO: Consider using a generator for the data = True case...it could be a lot of data
    ...
    def _set_betting_line_id(self, betting_line_id: str) -> str:
        ...
        b_id = utils.get_short_id(self._r, 'lines')
        self._r.hset(f'lines:lookup', key=betting_line_id, value=b_id)
        return b_id

    def _set_expirations(self, betting_line_id: str, bl_id: str, game_time: int) -> None:
        """Delete betting lines at game time"""
        self._r.hexpireat(f'lines:lookup', game_time, betting_line_id)
        self._r.hexpireat(f'lines:{bl_id}', game_time)
